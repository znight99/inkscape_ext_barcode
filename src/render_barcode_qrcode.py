...
class QrCode(inkex.GenerateExtension):
    def add_arguments(self, pars):
        ...
        pars.add_argument("--modulesize", type=float, default=1.0)
    ...
    def generate(self):
        ...
        if so.text == '':  # abort if converting blank text
            ...
            #inkex.errormsg(qr.modules)
            qrDraw = GridDrawer(size,1, False, so.smoothval, so.symbolid, 4)
