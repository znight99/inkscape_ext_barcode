...
class DataMatrix(inkex.GenerateExtension):
    ...
    def add_arguments(self, pars):
        ...
        pars.add_argument("--size", type=int, default=1)
    ...
    def generate(self):
        ...
        encoded = self.encode(text_bytes, *self.options.symbol)

        grp = Group()
        grp.set('inkscape:label', 'DataMatrix Code: ' + self.options.text)
        pos_x, pos_y = self.svg.namedview.center
        grp.transform.add_translate(pos_x, pos_y)

        for i in range(len(encoded)):
          for j in range(len(encoded[i])):
            for k in range(len(encoded[i][j])):
              encoded[i][j][k]=(encoded[i][j][k]!=0)
          #inkex.errormsg(encoded[0])

          ncol = self.options.symbol[1]
          size = self.options.size*self.svg.unittouu('1mm')
          spacing = ncol * size * 1.5

          grp1 = Group()
          grp1.set('inkscape:label', 'DM SubCode: %d'%(i+1))
          grp1.transform.add_translate(i*spacing, 0)


          qrDraw = GridDrawer(size, 1,False, self.options.smoothval, self.options.symbolid, 4)
          qrDraw.setGrid(encoded[i])
          qrDraw.makeSVG(grp1,self.options.drawtype)
          grp.append(grp1)
        return grp

        #inkex.errormsg(encoded)
