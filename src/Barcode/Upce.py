...
class Upce(EanBarcode):
    ...
    def _encode(self, n):
        ...
        self.label = self.space(['0'], 1, n[:6], 1, [n[-1]])
        code = self.encode_interleaved(n[-1], n[:6], FAMS)
        # 202(guard) + code + 020(center) + 202(guard)
        return '0'*9+self.enclose(code, center='020')+'0'*7

    def appendChecksum(self, number):
        """Generate a UPCE Checksum"""
        if len(number) == 6:
            number1 = self.ConvertEtoA(number)
        else:
            number1=number
        result = self.getChecksum(number1)
        return self.ConvertAtoE(number1) + result
