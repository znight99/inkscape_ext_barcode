...
#   */
def getRecommendedMinimumErrorCorrectionLevel(n):
    if (n <= 0):
      raise IllegalArgumentException("n must be > 0")
    if (n <= 40):
      return 2
    if (n <= 160):
      return 3
...
#   */
def generateErrorCorrection(dataCodewords, errorCorrectionLevel):
  k = getErrorCorrectionCodewordCount(errorCorrectionLevel)
  e = [ 0 ]*k
  sld = len(dataCodewords)
  for i in range(0,sld):
    t1 = (dataCodewords[i] + e[len(e) - 1]) % 929
