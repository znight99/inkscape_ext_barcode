...
class PDF417 :
  ...
  def __init__(self, compact):
    ...
    self.compaction = Compaction.AUTO
    self.encoding = None
    self.minCols = 2
    self.maxCols = 30
    self.maxRows = 90
  ...
  def  getBarcodeMatrix(self):
    return self.barcodeMatrix;
...
#   */
  def getNumberOfPadCodewords(self, m, k, c, r):
    n = c * r - k
    return n - m - 1 if n > m + 1 else 0
  
#
  def encodeChar(self, pattern,  xlen,  logic) :
    xmap = 1 << xlen - 1
    last = (pattern & xmap) != 0 #//Initialize to inverse of first bit
    width = 0
    for i in range(0, xlen) :
      black = (pattern & xmap) != 0
      if (last == black) :
        width+=1
      else :
    ...
    logic.addBar(last, width)
#
  def encodeLowLevel(self,fullCodewords, c, r, errorCorrectionLevel, logic) :
#
    idx = 0
    for y in range(0, r) :
      cluster = y % 3
...
#
      if (self.compact) :
        self.encodeChar(STOP_PATTERN, 1, logic.getCurrentRow())  #// encodes stop line for compact pdf417
      else :
        pattern = CODEWORD_TABLE[cluster][right]
        self.encodeChar(pattern, 17, logic.getCurrentRow())
#
        self.encodeChar(STOP_PATTERN, 18, logic.getCurrentRow())
#
#  /**
...
#   */
  def generateBarcodeLogic(self, msg, errorCorrectionLevel, aspect):

    self.aspect=aspect
    
#    //1. step: High-level encoding
    errorCorrectionCodeWords = PDF417ErrorCorrection.getErrorCorrectionCodewordCount(errorCorrectionLevel)
    #print errorCorrectionCodeWords
    highLevel = PDF417HighLevelEncoder.encodeHighLevel(msg, self.compaction, self.encoding)
    sourceCodeWords = len(highLevel)
#
    dimension = self.determineDimensions(sourceCodeWords, errorCorrectionCodeWords)
#
    cols = dimension[0];
    rows = dimension[1];
#
    pad = self.getNumberOfPadCodewords(sourceCodeWords, errorCorrectionCodeWords, cols, rows)
#
...
#    //3. step: Error correction
    ec = PDF417ErrorCorrection.generateErrorCorrection(dataCodewords, errorCorrectionLevel)
    fullCodewords = dataCodewords + ec;
#
#    //4. step: low-level encoding
    self.barcodeMatrix = BarcodeMatrix.BarcodeMatrix(rows, cols)
    self.encodeLowLevel(fullCodewords, cols, rows, errorCorrectionLevel, self.barcodeMatrix)
#
#  /**
...
#   */
  def determineDimensions(self,sourceCodeWords,  errorCorrectionCodeWords):
    ratio = 0.0
    dimension = []
    for cols in range( self.minCols, self.maxCols+1) :
      ...
      if (rows > self.maxRows):
        continue
      newRatio = float((17 * cols + 69) * 1.0) / (rows)
  #      // ignore if previous ratio is closer to preferred ratio
      if (dimension != [] and abs(newRatio - self.aspect) > abs(ratio - self.aspect)) :
        continue
      ratio = newRatio
      dimension = [cols, rows]
  #
  #     // Handle case when min values were larger than necessary
    if (dimension == []) :
      rows = self.calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, self.minCols)
      if (rows < self.minRows) :
        dimension = [self.minCols, self.minRows]
  #
    if (dimension == []) :
      raise WriterException("Unable to fit message in columns")
  #
    return dimension;
#
#  /**
...
#   */
  def setDimensions( self,maxCols,  minCols,  maxRows,  minRows) :
...
#
#p=PDF417(0)
#p.generateBarcodeLogic("Hello",0)
#m=p.barcodeMatrix.getMatrix()
