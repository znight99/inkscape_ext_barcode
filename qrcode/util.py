...
def optimal_data_chunks(data, minimum=4):
    ...
    re_repeat = '{' + unicode(minimum).encode('ascii') + ',}'
...
def is_kanji(data):
    try:
      data=unicode(data,'utf-8').encode('sjis')
    ...
    while i<len(data):
      ...
      if c>=0x81 and c<=0x9f or c>=0xe0 and c<=0xef:
        ...
        if c>=0x81 and c<=0x9f and c2>=0x40 and c2<=0xfc:
          pass
        elif c>=0xe0 and c<=0xeb and c2>=0x40 and c2<=0xfc:
          pass
        else:
          return False
        i+=2
...
def is_hanzi(data):
    ...
    while i<len(data):
      ...
      if c>=0x81 and c<=0xfe:
        c2=ord(data[i+1]) if i+1<len(data) else 0
        if c>=0xb0 and c<=0xfa and c2>=0xa1 and c2<=0xfe:
            pass
        elif c>=0xa1 and c<=0xaa and c2>=0xa1 and c2<=0xfe:
            pass
        else:
          return False
        i+=2
...
class QRData:
    ...
    def __init__(self, data, mode=None, check_data=True):
        ...
        if mode is None:
            ...
            self.mode = mode
            if mode not in (MODE_NUMBER, MODE_ALPHA_NUM, MODE_8BIT_BYTE, MODE_KANJI, MODE_HANZI, MODE_ECI, MODE_FNC1, MODE_FNC1_2):
    ...
    def write(self, buffer):
        if self.lensize<0:
            raise ValueError("Size of length is invalid")
        if self.mode == MODE_NUMBER:
            buffer.put(len(self.data),self.lensize)
        ...
        elif self.mode== MODE_KANJI: # input are encoded in SJIS
            data=unicode(self.data,'utf-8').encode('sjis')
            buffer.put(len(self.data),self.lensize)
            i=0
            st=True
            while i<len(data):
              ...
              if c>=0x81 and c<=0x9f or c>=0xe0 and c<=0xef:
                c2=ord(data[i+1]) if i+1<len(data) else 0
                if c>=0x81 and c<=0x9f and c2>=0x40 and c2<=0xfc:
                    buffer.put((c-0x81)*0xc0+(c2-0x40),13)
                elif c>=0xe0 and c<=0xeb and c2>=0x40 and c2<=0xfc:
                    buffer.put((c-0xc1)*0xc0+(c2-0x40),13)
                i+=2
              else:
                i+=1
        elif self.mode== MODE_HANZI: # input are encoded in GB2312
            ...
            while i<len(data):
              ...
              if c>=0x81 and c<=0xFE:
                c2=ord(data[i+1]) if i+1<len(data) else 0
                if c>=0xA1 and c<=0xAA and c2>=0xA1 and c2<=0xFE:
                    buffer.put((c-0xA1)*0x60+(c2-0xA1),13)
                elif c>=0xB0 and c<=0xFA and c2>=0xA1 and c2<=0xFE:
                    buffer.put((c-0xA6)*0x60+(c2-0xA1),13)
                i+=2
              else:
                i+=1
        ...
        elif self.mode == MODE_FNC1: # single FNC1 symbol, no data
            pass
        elif self.mode == MODE_FNC1_2: # with a GS1 Appl. ID
            buffer.put(int(self.data),8)
        else: # 8BIT_BYTE
            buffer.put(len(self.data),self.lensize)
...
def generate_data(buffer,data_list, version):
    if version is None:
      version=1
    for data in data_list:
        buffer.put(data.mode, 4)
        data.lensize=length_in_bits(data.mode, version)
        data.write(buffer)
