...
class GMData:
    ...
    def write(self, buffer):
        if self.mode==MODE_CHINESE:
          ...
          buffer.put(0,4) # GM code number-1
          buffer.put(0,4) # GM code seq no.
...
class BitBuffer:
    ...
    def put_bit(self, bit):
        buf_index = self.length // 7
        if len(self.buffer) <= buf_index:
            self.buffer.append(0)
        if bit:
            self.buffer[buf_index] |= (1 << (6-self.length % 7))
        self.length += 1

def create_bytes(num, buffer, rs_blocks):
    offset = 0
...
def create_data(version, error_correction, buffer):
    ...
    d=(len(buffer)+6)//7
    ec=error_correction
    if version==1 and ec==1:
      ec=2
    cm=(10*d+10-ec-1)//(10-ec)
    c=pow((2*version+1),2)*2
    ...
    return create_bytes(c,buffer, rbs)
